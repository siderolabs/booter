...
#
# Generated on 2026-07-30T12:22:13Z by kres 34f68df.
...
# source assets from ghcr.io/siderolabs/ipxe:v1.11.0
FROM --platform=linux/amd64 ghcr.io/siderolabs/ipxe:v1.11.0 AS source-assets-src-0

# source assets from ghcr.io/siderolabs/ipxe:v1.11.0
FROM --platform=linux/arm64 ghcr.io/siderolabs/ipxe:v1.11.0 AS source-assets-src-1
...
COPY --from=source-assets-src-0 /usr/libexec/ipxe.efi /internal/server/ipxe/data/amd64/ipxe.efi
COPY --from=source-assets-src-0 /usr/libexec/snp.efi /internal/server/ipxe/data/amd64/snp.efi
COPY --from=source-assets-src-0 /usr/libexec/kpxe/undionly.kpxe.bin /internal/server/ipxe/data/amd64/kpxe/undionly.kpxe.bin
COPY --from=source-assets-src-0 /usr/libexec/kpxe/undionly.kpxe.zinfo /internal/server/ipxe/data/amd64/kpxe/undionly.kpxe.zinfo
COPY --from=source-assets-src-1 /usr/libexec/ipxe.efi /internal/server/ipxe/data/arm64/ipxe.efi
COPY --from=source-assets-src-1 /usr/libexec/snp.efi /internal/server/ipxe/data/arm64/snp.efi
...
RUN --mount=type=cache,target=/root/.cache/go-build,id=booter/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=booter/go/pkg go install github.com/shanduur/dis-vulncheck@${DIS_VULNCHECK_VERSION} \
...
WORKDIR /src
RUN --mount=type=cache,target=/root/.cache/go-build,id=booter/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=booter/go/pkg dis-vulncheck -tool=false ./...
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=booter/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=booter/go/pkg GOARCH=amd64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=booter -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /booter-linux-amd64
...
ARG SHA
ARG TAG
RUN --mount=type=cache,target=/root/.cache/go-build,id=booter/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=booter/go/pkg GOARCH=arm64 GOOS=linux go build ${GO_BUILDFLAGS} -gcflags "${GO_GCFLAGS}" -ldflags "${GO_LDFLAGS} -X ${VERSION_PKG}.Name=booter -X ${VERSION_PKG}.SHA=${SHA} -X ${VERSION_PKG}.Tag=${TAG}" -o /booter-linux-arm64
