...
.PHONY: fetch-source-assets
fetch-source-assets:  ## Fetches the source assets into the source tree, for building natively outside docker.
	rm -rf -- 'internal/server/ipxe/data/amd64/ipxe.efi' 'internal/server/ipxe/data/amd64/snp.efi' 'internal/server/ipxe/data/amd64/kpxe/undionly.kpxe.bin' 'internal/server/ipxe/data/amd64/kpxe/undionly.kpxe.zinfo' 'internal/server/ipxe/data/arm64/ipxe.efi' 'internal/server/ipxe/data/arm64/snp.efi'
	@$(MAKE) local-source-assets DEST=./ PLATFORM=linux/amd64
...
.PHONY: sign-image-booter
sign-image-booter:  ## Signs the image for booter. Requires interactive Google authentication.
	@test -n "$$GITHUB_TOKEN" || { \
	...
	@TMP=$$(mktemp -d) && trap 'rm -rf "$$TMP"' EXIT && \
	  ...
	  docker run --rm -p 127.0.0.1:8585:8585 \
	    ...
	    $(REGISTRY)/$(USERNAME)/booter:$(IMAGE_TAG)
